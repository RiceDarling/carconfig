...
vec2 normalToPanoramaXRender( const in vec3 dir )
{
	vec3 reflectDir = (vec4(dir,0.0)).xyz;
	float m = 2.0 *length(reflectDir);

	float angle = acos(dot(vec3(1.,0.,0.),normalize(vec3(reflectDir.x,reflectDir.y,0.0))));
	if(reflectDir.y>0.)
	{
		angle = 2.0*PI - angle;
	}
	vec2 uv =  vec2(angle/(2.0*PI),reflectDir.z/2.0+0.5);
